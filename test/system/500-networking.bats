...
@test "podman run port forward range" {
    ...
    for netmode in "${netmodes[@]}"; do
        ...
        assert "$status" -eq 2 "nc unexpected exit code"
        assert "$output" =~ "127.0.0.1:$port: Address already in use" "nc error message"

        for port in $(seq $port $end_port); do
            run_podman exec -d $cid nc -l -p $port -e /bin/cat

            # we have to rety nc as it can flake as we exec in the background so nc -l
            ...
            is "$output" "$random" "nc got data back (netmode=$netmode port=$port)"
        done
